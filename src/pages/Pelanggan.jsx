...
export default function PelangganPage() {
  ...
  const handleAdd = async (e) => {
    ...
    setLoading(true)
    // Ambil User ID yang lagi login biar lolos RLS Supabase Multi-Tenant
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={() => setShowAdd(!showAdd)} style={{ background: '#4361EE', color: 'white', padding: '10px 18px', borderRadius: '12px', fontWeight: '700', cursor: 'pointer', border: 'none' }}>
          {showAdd ? 'Batal' : '+ Pelanggan Baru'}
        </button>
      </div>
      ...
      {showAdd && (
        <form onSubmit={handleAdd} style={{ background: 'white', padding: '16px', borderRadius: '16px', border: '1px solid #eef2f7', marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <input placeholder="Nama Pelanggan *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} style={{ padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }} required />
          <input placeholder="No WA (08...) *" value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} style={{ padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }} required />
          <input placeholder="Alamat Jemput / Rumah" value={form.address} onChange={e => setForm({ ...form, address: e.target.value })} style={{ padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' }} />
          <button type="submit" disabled={loading} style={{ background: '#111', color: 'white', padding: '12px', borderRadius: '10px', fontWeight: '700', cursor: 'pointer', opacity: loading ? 0.7 : 1 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div style={{ display: 'flex', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '13px', color: '#64748b' }}>{c.phone} • {c.address ? c.address.slice(0, 20) : 'Tanpa alamat'}</div>
                  </div>
                </div>
                <button onClick={() => handleDelete(c.id)} style={{ border: 'none', background: '#f8fafc', width: '30px', height: '30px', borderRadius: '8px', cursor: 'pointer', color: '#ef4444', fontWeight: '700' }}>✕</button>
              </div>
